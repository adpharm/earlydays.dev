...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  const posts = await db.select().from(postsTable).where(eq(postsTable.isPublished, true));
...
Github
</a> </div> ${post.tags && post.tags !== "" && renderTemplate`<div class="flex flex-row justify-center items-center"> <div class="flex flex-row justify-center items-center space-x-1 w-1/2"> ${post.tags.split(",").map((tag) => renderTemplate`<div class="flex justify-center items-center text-sm px-4 py-1 rounded-full bg-indigo-200 text-indigo-600"> ${tag} </div>`)} </div> </div>`} </div>`;
  })()} </div> </div>`)} </div> </div> ${renderScript($$result2, "/workspace/src/pages/index.astro?astro&type=script&index=0&lang.ts")} ` })}`;
